...
async def pdf_view_proxy(url: str = ""):
    """Serve a lightweight HTML page that embeds the PDF for inline viewing (iPad/Safari compatible)."""
    if not url:
...
<style>*{{margin:0;padding:0}}body{{background:#525659;display:flex;flex-direction:column;height:100vh}}
.toolbar{{background:#1a2332;padding:8px 16px;display:flex;align-items:center;gap:12px}}
.toolbar a{{color:#fff;text-decoration:none;font:14px system-ui;padding:6px 14px;border-radius:6px;background:rgba(255,255,255,.15)}}
.toolbar a:hover{{background:rgba(255,255,255,.25)}}
.toolbar span{{color:rgba(255,255,255,.7);font:13px system-ui;flex:1}}
iframe{{flex:1;border:none}}</style></head>
<body>
<div class="toolbar">
<a href="javascript:history.back()">← Zurück</a>
<span>BEATRIX Paper Viewer</span>
<a href="{safe_url}" download>⬇ Download</a>
</div>
<iframe src="{safe_url}" width="100%" height="100%"></iframe>
</body></html>"""
